...
#define lock_status_channel "openhab/in/Main_door/state"
#define HOSTNAME "main_door"
...
void onMqttConnect(bool sessionPresent) {
  ...
  attachInterrupt(digitalPinToInterrupt(BELL), sendAlert, FALLING);
}

void onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
  Serial.println("Disconnected from MQTT.");
  detachInterrupt(BELL);
  ledOff();
  if (WiFi.isConnected()) {
...
void triggerSwitch() {
  mqttClient.publish(lock_status_channel, 0, false, "ON");
  digitalWrite(RELAY, LOW);
  switchTimer.once(0.5, [] () {
    relayOff();
    mqttClient.publish(lock_status_channel, 0, false, "OFF");
  });
}
